        ...
        try:
            ...
            if len(patterns) > 0:
                ...
                for pattern in patterns:
                    ...
                    # ===================================================================
                    # SPECIAL: Draw colored price line for ALL patterns
                    # ===================================================================
                    
                    # Get price segment for this pattern
                    pattern_price_segment = df[price_col].iloc[start_idx:end_idx+1]
                    pattern_date_segment = df.index[start_idx:end_idx+1]
                    
                    # Choose color based on pattern type and direction
                    if pattern_type == 'Head & Shoulders':
                        line_color = 'red'
                        line_label = 'H&S Pattern'
                    elif pattern_type == 'Inverse H&S':
                        line_color = 'lime'
                        line_label = 'Inverse H&S'
                    elif pattern_type == 'Double Top':
                        line_color = 'darkred'
                        line_label = 'Double Top'
                    elif pattern_type == 'Double Bottom':
                        line_color = 'green'
                        line_label = 'Double Bottom'
                    elif pattern_type == 'Ascending Triangle':
                        line_color = 'limegreen'
                        line_label = 'Asc Triangle'
                    elif pattern_type == 'Descending Triangle':
                        line_color = 'crimson'
                        line_label = 'Desc Triangle'
                    elif pattern_type == 'Symmetrical Triangle':
                        line_color = 'gold'
                        line_label = 'Sym Triangle'
                    elif pattern_type == 'Cup & Handle':
                        line_color = None  # Will be handled specially below
                        line_label = None
                    else:
                        line_color = outline_color  # Fallback
                        line_label = pattern_type
                    
                    # Draw colored price line for non-Cup patterns
                    if line_color is not None:
                        ax.plot(pattern_date_segment, pattern_price_segment,
                               color=line_color, linewidth=2, linestyle='-',
                               alpha=1.0, zorder=10,
                               label=line_label if start_idx == pattern['start_idx'] else '')
                        print(f"      ✅ Drew {pattern_type} in {line_color.upper()}")
                    
                    # ===================================================================
                    # SPECIAL: Draw Cup Shape for Cup & Handle Patterns
                    # ===================================================================
                    if pattern_type == 'Cup & Handle':
                        print(f"   🏆 Found Cup & Handle pattern!")
                        print(f"      Has cup_left_idx: {'cup_left_idx' in pattern}")
                        
                        if 'cup_left_idx' in pattern:
                            ...
                            print(f"      Cup dates: {cup_left_date} → {cup_bottom_date} → {cup_right_date}")
                            print(f"      Drawing lime green cup...")
                            ...
                            ax.plot(left_cup_dates, left_cup_segment,
                                   ...
                                   alpha=1.0, zorder=10, label='Cup' if start_idx == pattern['start_idx'] else '')
                            ...
                            ax.plot(handle_dates, handle_segment,
                                   ...
                                   alpha=1.0, zorder=10, label='Handle' if start_idx == pattern['start_idx'] else '')
                            
                            print(f"      ✅ Cup drawn in LIME GREEN!")
                            print(f"      ✅ Handle drawn in ORANGE!")
                            ...
                            ax.plot(cup_bottom_date, cup_bottom_price, 'o',
                                   color='lime', markersize=12, markeredgecolor='darkgreen',
                                   markeredgewidth=2, zorder=12)
                            ...
                            ax.plot(handle_start_date, df[price_col].iloc[handle_start_idx], 's',
                                   color='orange', markersize=10, markeredgecolor='darkorange',
                                   markeredgewidth=2, zorder=12)
                            
                            print(f"      ✅ Markers and labels added!")
                        else:
                            print(f"      ⚠️  Cup structure data missing!")
