...
def show_watchlist_stocks_enhanced(watchlist_id):
    ...
    if st.session_state.get(f"analyzing_{watchlist_id}", False):
        ...
        for idx, symbol in enumerate(stocks):
            ...
            # Force analysis (will use cache if available)
            analyze_stock_enhanced(symbol, data_source, columns_to_show)
        
        progress_bar.empty()
        status_text.empty()
        st.session_state[f"analyzing_{watchlist_id}"] = False
        st.success(f"✅ Analyzed {len(stocks)} stocks!")
        st.rerun()
    
    # Build stock data
    stock_data = []
    for symbol in stocks:
        data = analyze_stock_enhanced(symbol, data_source, columns_to_show)
        if data:
