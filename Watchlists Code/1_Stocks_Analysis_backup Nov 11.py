...
if 'stock_info' in st.session_state and 'stock_data' in st.session_state:
    ...
    if not df.empty:
        ...
        # Today's Price Range bar (below metrics)
        ...
        if not df.empty:
            ...
            # Create single-line horizontal bar
            fig_today = go.Figure()
            
            # Red bar from low to current price (left of price)
            ...
            # Green bar from current price to high (right of price)
            ...
            # Current price marker (blue diamond)
            ...
            fig_today.update_layout(
                ...
                xaxis=dict(
                    tickmode='array',
                    tickvals=[today_low, today_high],
                    ticktext=[f'${today_low:.2f}', f'${today_high:.2f}'],
    ...
    with col_right:
        ...
        def calculate_returns(df, periods):
            ...
            # YTD
            try:
                df_indexed = df.reset_index() if 'Date' not in df.columns else df
                current_year = df_indexed['Date'].iloc[-1].year
                year_data = df_indexed[df_indexed['Date'].dt.year == current_year]
                if len(year_data) > 0:
                    year_start_price = year_data.iloc[0]['Close']
        ...
        if not df.empty:
            ...
            max_pct = 35  # Fixed max for consistent bar widths
            ...
            for i, (label, price, pct, color, annot) in enumerate(chart_data):
                ...
                display_pct = max(pct, 3) if pct > 0 else 3  # Minimum 3% width for visibility
                ...
                fig.add_trace(go.Bar(
                    ...
                    hoverinfo='skip',
                    showlegend=False,
                    width=0.8  # Increase bar thickness
                ))
