...
def show_watchlist_stocks_enhanced(watchlist_id):
    ...
    # ============= FILTERS SECTION =============
    st.markdown("---")
    st.markdown("**🔍 Filters:** *(Set filters below, then click 'Apply Filters' button)*")
    
    # Initialize filter state
    filter_key = f"filters_{watchlist_id}"
    if filter_key not in st.session_state:
        st.session_state[filter_key] = {
            ...
            'perf_column': 'perf_ytd',
            'active': False
        }
    
    filter_col1, filter_col2, filter_col3, filter_col4, filter_col5 = st.columns([1.5, 1.5, 2, 2, 1])
    ...
    with filter_col1:
        ...
        price_low = st.number_input(
            ...
            min_value=0,
            max_value=100000,
            value=st.session_state[filter_key]['price_min'],
            step=10,
            key=f"price_low_{watchlist_id}"
        )
        st.session_state[filter_key]['price_min'] = price_low
    ...
    with filter_col2:
        ...
        price_high = st.number_input(
            ...
            min_value=0,
            max_value=100000,
            value=st.session_state[filter_key]['price_max'],
            step=10,
            key=f"price_high_{watchlist_id}"
        )
        st.session_state[filter_key]['price_max'] = price_high
    
    with filter_col3:
        # TR Status filter (only if TR Status is in view)
        if 'tr_status' in columns_to_show:
            tr_filter = st.multiselect(
                "TR Status",
                options=['Strong Buy', 'Buy', 'Neutral', 'Sell', 'Strong Sell'],
                key=f"tr_filter_{watchlist_id}",
                help="Filter by TR (Trend Recognition) indicator signal"
            )
            st.session_state[filter_key]['tr_status'] = tr_filter
        else:
            st.session_state[filter_key]['tr_status'] = []
    
    with filter_col4:
        # Performance filter - show time period selector first
        perf_cols = [c for c in columns_to_show if 'perf_' in c]
        if perf_cols:
            # Two-row selection: Time Period + Performance Type
            perf_time_options = {
                'perf_1m': '1 Month',
                'perf_3m': '3 Months', 
                'perf_6m': '6 Months',
            ...
            # Only show time periods that are in the current view
            available_time_periods = {k: v for k, v in perf_time_options.items() if k in columns_to_show}
            
            if available_time_periods:
                perf_column = st.selectbox(
                    "Performance Time Period",
                    options=list(available_time_periods.keys()),
                    format_func=lambda x: available_time_periods[x],
                    index=0,
                    key=f"perf_column_{watchlist_id}"
                )
                st.session_state[filter_key]['perf_column'] = perf_column
                ...
                perf_filter = st.selectbox(
                    ...
                    key=f"perf_filter_{watchlist_id}"
                )
                st.session_state[filter_key]['perf_filter'] = perf_filter
            else:
                st.session_state[filter_key]['perf_filter'] = 'All Stocks'
    
    with filter_col5:
        st.markdown("<br>", unsafe_allow_html=True)
        # Apply button - forces filtering immediately
        if st.button("✅ Apply Filters", key=f"apply_filters_{watchlist_id}", type="primary", use_container_width=True):
            # Just trigger a rerun - filters will be applied to existing data
            st.rerun()
        
        # Clear button below Apply
        if st.button("🔄 Clear", key=f"clear_filters_{watchlist_id}", use_container_width=True):
            st.session_state[filter_key] = {
                'price_min': 0,
                'price_max': 10000,
                'tr_status': [],
            ...
            # Force clear any displayed results too
            st.rerun()
    
    # Check if any filters are active (check session state, not just UI values)
    filters_active = (
        st.session_state[filter_key]['price_min'] != 0 or 
        st.session_state[filter_key]['price_max'] != 10000 or
        st.session_state[filter_key]['tr_status'] or
        st.session_state[filter_key].get('perf_filter', 'All Stocks') != 'All Stocks'
    )
    st.session_state[filter_key]['active'] = filters_active
    
    # Show active filter summary
    if filters_active:
        filter_summary = []
        if price_low != 0 or price_high != 10000:
            filter_summary.append(f"💰 Price: ${price_low:,.0f}-${price_high:,.0f}")
        if st.session_state[filter_key]['tr_status']:
            tr_list = ", ".join(st.session_state[filter_key]['tr_status'])
            filter_summary.append(f"📊 TR: {tr_list}")
        if st.session_state[filter_key].get('perf_filter', 'All Stocks') != 'All Stocks':
            perf_time = st.session_state[filter_key].get('perf_column', 'perf_ytd')
            perf_time_name = {
                'perf_1m': '1M',
                'perf_3m': '3M', 
                'perf_6m': '6M',
            ...
            perf_type = st.session_state[filter_key]['perf_filter']
            filter_summary.append(f"📈 {perf_type} in {perf_time_name}")
        
        st.info(f"🔍 **Active Filters:** {' • '.join(filter_summary)} | Click 'Clear All' to reset")
    else:
        st.caption("ℹ️ No filters active. Select filters above then click 'Analyze All' to apply.")
    ...
    # ============= APPLY FILTERS =============
    if st.session_state[filter_key]['active']:
        filtered_data = []
        ...
        for stock in stock_data:
            ...
            if price and isinstance(price, (int, float)):
                if price < st.session_state[filter_key]['price_min'] or price > st.session_state[filter_key]['price_max']:
                    continue
            
            # TR Status filter
            if st.session_state[filter_key]['tr_status']:
                tr_status = stock.get('tr_status')
                if tr_status not in st.session_state[filter_key]['tr_status']:
                    continue
            
            # Performance filter - now uses specific time period
            perf_filter = st.session_state[filter_key].get('perf_filter', 'All Stocks')
            if perf_filter != 'All Stocks':
                # Get the specific performance column user selected
                perf_column = st.session_state[filter_key].get('perf_column', 'perf_ytd')
                
                # Get value from that specific column
                perf_value = None
                val = stock.get(perf_column)
                
                # DEBUG: Show what we're looking for
                st.caption(f"🔍 {stock.get('symbol')}: column '{perf_column}' = '{val}'")
                
                # Handle both formats: "+11.2%" (string) or 11.2 (number) or "+11.198..." (string number)
                if val is not None:
                    try:
                        if isinstance(val, (int, float)):
                            # Already a number
                            perf_value = float(val)
                        elif isinstance(val, str):
                            # String - remove %, +, commas and convert
                            perf_value = float(val.replace('%', '').replace('+', '').replace(',', ''))
                    except:
                        pass
                
                # DEBUG: Show parsed value
                # st.write(f"DEBUG: Parsed value: {perf_value}")
                
                if perf_value is not None:
                    skip_stock = False
                    
                    if perf_filter == 'Gainers (positive)' and perf_value <= 0:
                        skip_stock = True
                    elif perf_filter == 'Losers (negative)' and perf_value >= 0:
                        skip_stock = True
                    elif perf_filter == 'Up >5%' and perf_value <= 5:
                        skip_stock = True
                    elif perf_filter == 'Up >10%' and perf_value <= 10:
                        skip_stock = True
                    elif perf_filter == 'Down >5%' and perf_value >= -5:
                        skip_stock = True
                    elif perf_filter == 'Down >10%' and perf_value >= -10:
                        skip_stock = True
                    
                    # DEBUG: Show decision
                    # st.write(f"DEBUG: Skip {stock.get('symbol')}? {skip_stock}")
                    
                    if skip_stock:
                        continue
                else:
                    # If perf_value is None (couldn't parse), skip this stock when filtering
                    continue
            
            # Stock passed all filters
            filtered_data.append(stock)
        
        # Replace stock_data with filtered data
        original_count = len(stock_data)
        stock_data = filtered_data
        
        if len(stock_data) < original_count:
            st.caption(f"📊 Showing {len(stock_data)} of {original_count} stocks (filtered)")
