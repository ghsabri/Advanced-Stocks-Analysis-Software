...
<style>
    ...
    /* Compact table rows - reduce vertical spacing */
    div[data-testid="stVerticalBlock"] > div[data-testid="stVerticalBlockBorderWrapper"] {
        padding: 0 !important;
        margin: 0 !important;
    }
    
    /* Reduce spacing between horizontal blocks (table rows) */
    ...
    .compact-table-row .stButton > button {
...
def display_stock_table(stocks_data, list_key, data_loaded=False):
    """Display compact stock table with clickable symbols and sorting"""
    ...
    # Column definitions
    col_widths = [1.1, 1, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9]
    col_ids = ['symbol', 'price', 'daily_change', 'perf_1w', 'perf_1m', 'perf_3m', 'perf_6m', 'perf_ytd', 'perf_1y']
    col_names = ['Symbol', 'Price', 'Daily', '1 Week', '1 Month', '3 Months', '6 Months', 'YTD', '1 Year']
    
    # Header row with sort buttons
    header_cols = st.columns(col_widths)
    for col, col_id, col_name in zip(header_cols, col_ids, col_names):
    ...
    for stock in stocks_data:
        ...
        # Symbol (clickable)
        with row_cols[0]:
            if st.button(symbol, key=f"sym_{list_key}_{symbol}", type="tertiary", use_container_width=True):
        ...
        with row_cols[1]:
            ...
            st.markdown(f"<div style='text-align:center;padding-top:5px;font-size:13px'>{price}</div>", unsafe_allow_html=True)
        ...
        for col, key in zip(row_cols[2:], perf_keys):
            ...
            col.markdown(f"<div style='text-align:center;padding-top:5px;font-size:13px;color:{color}'>{formatted}</div>", unsafe_allow_html=True)
