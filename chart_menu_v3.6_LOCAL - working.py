"""
TR CHART MENU v3.6 - Select Timeframe AND Duration
Two-step selection: 1) Timeframe (Daily/Weekly), 2) Duration
NOW WITH REAL DATA + CACHING!
Version 3.6 - FIXED: Function order + CLEAR CACHE for split stocks

FILE: chart_menu_v3.4_LOCAL.py (will be renamed to v3.6)

CRITICAL v3.6:
- ✅ Fixed function definition order (wrapper after split functions)
- ✅ Split adjustment now works correctly
- 🚨 IMPORTANT: Clear cache (option 3) after upgrading to get fixed data!
...
def analyze_stock_complete_tr_with_split_adjustment(ticker, timeframe='daily', duration_days=180):
    """
    Wrapper around TR system that adjusts splits BEFORE TR calculations
    
    This ensures TR indicators are calculated on split-adjusted data
    """
    ...
    # CRITICAL: Adjust for splits BEFORE any TR calculations
    print(f"\n🔍 Checking for stock splits...")
    df = check_and_adjust_splits(df, ticker)
    
    # Now calculate TR indicators on split-adjusted data
    ...
    try:
        from tr_enhanced import (
            ...
            add_volume_analysis,
            enhance_tr_status,
            apply_status_continuity_rules
        )
        
        df = add_peaks_and_valleys(df)
        df = calculate_buy_points(df)
        
        # Try to add market data for RS
        try:
            market_df = get_stock_data_formatted('SPY', timeframe=timeframe, days=duration_days, save=False)
            if market_df is not None:
                df = add_relative_strength(df, market_df)
                df = add_volume_analysis(df)
        except:
            print("   ⚠️  Could not calculate RS (market data unavailable)")
        
        df = enhance_tr_status(df)
        df = apply_status_continuity_rules(df)
        
    except ImportError as e:
        print(f"   ⚠️  Some enhancements unavailable: {e}")
...
def draw_tr_chart_v2(ticker='AAPL', timeframe='Daily', duration='1 Year', show_chart=True):
    ...
    if USE_REAL_DATA:
        ...
        if df is None:
            ...
            try:
                # Call TR enhanced with SPLIT ADJUSTMENT BUILT-IN
                ...
                if df is not None and len(df) > 0:
                    print(f"   ✅ Analysis complete with {len(df)} rows")
    ...
    if 'TR_Status' in df.columns:
        ...
        for status in unique_statuses:
            if pd.notna(status):  # Skip nan values in counting
                count = (df['TR_Status'] == status).sum()
                print(f"   • {status}: {count} occurrences")
            else:
                count = df['TR_Status'].isna().sum()
                print(f"   • nan: {count} occurrences")
        
        # Check if we have any Stage 2 (Buy/Sell) data
        buy_count = (df['TR_Status'] == 'Buy').sum()
        sell_count = (df['TR_Status'] == 'Sell').sum()
        
        if buy_count == 0 and sell_count == 0:
            print(f"\n⚠️  WARNING: No Stage 2 data found!")
            print(f"   • 'Buy' (Stage 2 Uptrend): 0 occurrences")
            print(f"   • 'Sell' (Stage 2 Downtrend): 0 occurrences")
            print(f"   This stock may not have entered Stage 2 during this period.\n")
        elif buy_count < 5 or sell_count < 5:
            print(f"\n⚠️  Note: Very few Stage 2 occurrences (Buy: {buy_count}, Sell: {sell_count})")
            print(f"   Stage 2 bands may be very small or brief.\n")
        else:
            print(f"\n✅ Stage 2 data found: Buy={buy_count}, Sell={sell_count}\n")
    ...
    for i in range(len(df)):
        ...
        if pd.isna(status):
            # If we were drawing a band, end it
        ...
        if status in stage_colors:
            ...
            # Status is not in stage_colors (Neutral, Neutral Buy, Neutral Sell)
            if current_status is not None:
                band_end = i - 1
                if band_end >= band_start:
                    x_start = df.iloc[band_start]['Date']
