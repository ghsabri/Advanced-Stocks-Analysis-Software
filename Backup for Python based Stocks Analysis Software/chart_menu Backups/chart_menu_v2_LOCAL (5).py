...
Version 3.1 - Added stock split detection and warnings
...
def check_and_warn_splits(df, ticker):
    """
    Check for splits and warn user if found
    
    Args:
        df: DataFrame with stock data
        ticker: Stock symbol
    """
    splits = detect_stock_splits(df, threshold=0.25)
    
    if splits:
        print(f"\n⚠️  Detected {len(splits)} potential split(s) in {ticker}:")
        for split in splits:
            ratio = split['ratio']
            date = split['date']
            print(f"   • {date}: ~{ratio:.1f}-for-1 split (${split['prev_price']:.2f} → ${split['curr_price']:.2f})")
        
        print("   ℹ️  Tiingo should auto-adjust splits. Check chart for gaps.\n")
...
def draw_tr_chart_v2(ticker='AAPL', timeframe='Daily', duration='1 Year', show_chart=True):
            ...
            try:
                ...
                if df is not None and len(df) > 0:
                    ...
                    if len(df) > days:
                        ...
                        print(f"   ✅ Trimmed to {len(df)} rows for display (EMAs calculated on full dataset)")
    ...
    check_and_warn_splits(df, ticker)
