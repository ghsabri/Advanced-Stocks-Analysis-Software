    ...
    try:
        ...
        if len(patterns) > 0:
            ...
            for pattern in patterns:
                ...
                # Draw semi-transparent box around pattern
                box_height = ymax - ymin
                box_bottom = ymin
                
                ax.axvspan(start_date, end_date, 
                          ymin=0, ymax=1,
                          alpha=0.1, color=box_color, zorder=1)
                
                # Add vertical lines at start and end
                ax.axvline(start_date, color=box_color, linestyle=':', 
                          linewidth=1, alpha=0.5, zorder=2)
                ax.axvline(end_date, color=box_color, linestyle=':', 
                          linewidth=1, alpha=0.5, zorder=2)
                
                # Add pattern label at top
                label_y = ymax * 0.98  # Near top of chart
                ...
                if 'target_price' in pattern:
                    ...
                    ax.axhline(target, xmin=0.02, xmax=0.98,
                              color=box_color, linestyle='--', 
                              linewidth=1, alpha=0.4, zorder=2)
                    
                    # Add target label
                    ax.text(df.index[-1], target, f' Target: ${target:.2f}',
